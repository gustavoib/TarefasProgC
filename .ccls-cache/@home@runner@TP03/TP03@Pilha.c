...
int pilha_push(Pilha *pilha, Aluno *aluno) {
  ...
  for (int i = 0; i < pilha->tamanho; i++) {
    alu_acessa((&(pilha->pilha_alunos))[i], &matriculacmp, nomecmp, cursocmp);
    if (matriculacmp == matricula) {
      return 0;
    }
  }
...
Aluno *pilha_pop(Pilha *pilha) {
  ...
  Aluno *aluno_pop = (&(pilha->pilha_alunos))[tam - 1];
  (&(pilha->pilha_alunos))[tam - 1] = NULL;
  
  pilha->tamanho--; // aqui nao se usa a var tam, pois o valor dela esta estritamente ligado a função, tornando impossivel o decremento da pilha;
